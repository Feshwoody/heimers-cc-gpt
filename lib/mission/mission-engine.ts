...
export function runMissionEngine(input:MissionEngineInput):MissionEngineOutput {
  ...
  if(input.manualMission&&!input.manualMission.acknowledged&&(input.now??Date.now())-input.manualMission.createdAt<15_000){const fallback=input.manualMission.title==="MID SS"?["MID FEHLT","NICHT ÜBERDEHNEN"]:input.manualMission.title.startsWith("JUNGLE")?["JUNGLE-GEFAHR","SICHER SPIELEN"]:["Gemeinsam reagieren"];missions.push({id:input.manualMission.id,type:"manual",priority:priorityFor("manual"),severity:"urgent",title:input.manualMission.title,instructions:(input.manualMission.instructions??fallback).slice(0,settings.maxInstructions),createdAt:new Date(input.manualMission.createdAt).toISOString(),expiresAt:new Date(input.manualMission.createdAt+15_000).toISOString(),source:"manual"})}
