...
import { shouldResetForGame } from "@/lib/commander/match-ui-state";
...
export function CommanderShell({code,secret,game,call,shared,matchState,matchSource,members,botlaneEnemy,onCall,onShare,onResetForGame,onStartNewMatch,onCreateNewSession,onLeave}:Props){
  const [dismissedEndedId,setDismissedEndedId]=useState(""),[confirmNew,setConfirmNew]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(""),endedKey=matchState.gameId??"ended",canonicalState=matchState.phase==="active"?"ingame":matchState.phase==="waiting"?"prematch":matchState.phase,state=canonicalState==="ended"&&dismissedEndedId===endedKey?"prematch":canonicalState,previousGameId=useRef<string|undefined>(undefined),age=matchState.updatedAt?Math.max(0,Math.round((Date.now()-new Date(matchState.updatedAt).getTime())/1000)):0,companionOnline=members.some(member=>member.role==="heimer");
  ...
  return <main className={`commander-shell commander-${state}`}>
    ...
    {process.env.NODE_ENV!=="production"&&<small>CANONICAL MATCH STATE: sessionCode {code} · gameId {matchState.gameId??"–"} · gameTime {matchState.gameTime??"–"} · phase {matchState.phase} · connectorStatus {matchState.connectorStatus} · updatedAt {matchState.updatedAt??"–"} · enemyPlayerCount {game?.enemyPlayers?.length??0} · source {matchSource}</small>}
