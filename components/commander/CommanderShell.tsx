...
type Props={code:string;secret:string;game?:NormalizedGameData&{status?:string};call?:DuoCall;shared?:SharedState;members:SessionMember[];botlaneEnemy:string[];onCall:(text:string)=>void;onShare:(state:SharedState)=>void;onResetForGame:()=>void;onStartNewMatch:()=>void;onCreateNewSession:()=>Promise<void>;onLeave:()=>void};
export function CommanderShell({code,secret,game,call,shared,members,botlaneEnemy,onCall,onShare,onResetForGame,onStartNewMatch,onCreateNewSession,onLeave}:Props){
